...
interface Finding {
  kind: 'write' | 'edit' | 'gate' | 'command' | 'reuse' | 'special' | 'claim';
  key: string;
  path: string;
  test: string;
  block: boolean;
...
async function callTypeSafe(
  deps: ReviewDeps,
  settings: Settings,
  batch: Batch | EditRequest | GateRequest | SpecialRequest | OverrideRequest | SentenceRequest | RuleRequest | ReuseRequest | ClaimRequest,
  allowed: string,
...
const NOUNS: Record<Finding['kind'], string> = { write: 'test', edit: 'test', gate: 'change', command: 'command', reuse: 'function', special: 'change', claim: 'claim' };
