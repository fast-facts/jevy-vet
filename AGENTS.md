...
OpenCode plugin that vets agent writes with TypeSafe Jev. Test checks, the check-weakening check, and the special-case check block. The instruction check and the reuse check only warn. It calls the API itself. It does not use the Jev MCP server.
...
- `src/subjects.ts` — the new test text from `write`, `edit`, and `apply_patch`, split into setup and one case per test. Also the old and new text of each changed test, for the edit check, and of each changed file of any kind, for the instruction check. Which files set a check, the `bash` command, and which commands are in scope for the check-weakening check. The function-like definitions in a source file, for the reuse check. Test helper paths and the literals in a text, for the special-case check.
- `src/subjects.test.ts` — splitting, title, check file, command scope, definition, literal, and test helper tests.
- `src/context.ts` — the setup and the code under test read from the project folder, the instruction files OpenCode loads, split into sentences, the project's source files for the reuse check, and the tests related to a source file for the special-case check. Context only.
- `src/context.test.ts` — code-under-test lookup, instruction file, sentence, source file, and related test tests.
- `src/review.ts` — what to judge, the TypeSafe call, the block or note decision, the user's allow, the check-weakening check, the special-case check, the instruction check, and the reuse check.
- `src/review.test.ts` — block decision, check-weakening, special-case, instruction check, and reuse check tests.
...
- Old text may be used only as contrast evidence for the edit check, the check-weakening check, the special-case check, and the instruction check: `oldString`, the removed lines of a patch, a deleted test file, or the file on disk before a `write`. Never send it with the new-test questions. The reuse check uses it only to tell new functions from changed or moved ones, and never sends it.
- Strip code comments from old and new text before the edit check, the special-case check, and the instruction check. Jev judges the code, not the explanation. For the instruction check, only strip files whose comment syntax is known. Prose like "don't" is not a quote.
...
- A block or note names, per test: the file, the test, the rule with one plain line, the evidence, and the next step. Evidence is copied from the text, never made up: assertion or mock lines for a new test, the old and new lines for an edit or a check file, the special-cased line and the matching test line for a special case, and the command for `bash`. List at most five.
...
- The user-intent question and the allow question are not the same. User intent asks if the user asked for this edit, from recent messages, before any block. It covers test edits, check files, commands, special cases, and new functions, built by one helper. Allow asks if the user allowed a block, from messages after it, for every kind of block. Both allow at 0.5 and log it. Do not add a third.
...
- The first checks are tests, then instructions, then checks that CI and tests enforce, then reuse, then special cases. Add later checks in `src/review.ts`. Do not rename the package for a new check.
