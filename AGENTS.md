...
- A note-only check never blocks. It notes only at 0.8, in `tool.execute.after` when the note is ready within 1.5s, else on the next tool result in the same top session as `Note on your earlier change to <path>:`. A late note is dropped on the next user message or after 5 minutes. Below 0.8 it says nothing. With no key, an unreadable config, or a TypeSafe failure, it adds nothing.
- Identical TypeSafe bodies share one cached answer keyed by the exact JSON body, in memory: 500 entries oldest-first, 30 minutes each, never a failure. A changed body misses.
...
- Only real user text counts: `chat.message` text and a finished `question`-tool answer. Synthetic parts, ignored parts, and subagent prompts do not. A subagent's `question` answer counts on its parent session. A subagent's edit uses the parent session's user messages.
- User intent and allow are not the same. Both allow at 0.5, or drop the note. A `question`-tool answer is user text for both, not a third path. Do not add a third. User intent is one shared helper, asked before any block. Allow is asked after a block, from later real messages in the top-level session.
...
- Cut each instruction file to 8,000 characters and all of them to 24,000. Split sentences in code. Ask once whether a sentence is a rule, and cache that answer. User sentences are classified early through one shared helper: at the first tool call of the turn by default, or in `chat.message` with `CLASSIFY_ON_MESSAGE` on. The flag-on path reads the key before a write is about to happen. Open question whether that breaks the key rule. Do not cache a missing answer. A later user message can lift a rule. Keep the newest rules when capping.
